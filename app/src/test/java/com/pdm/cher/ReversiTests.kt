...
class ReversiTests {
    ...
    fun `makePlay should update board and switch player for a valid single switch move`() {
        ...
        assertEquals(Color.BLACK, newGame.board[2][4].color)
        assertEquals(Color.BLACK, newGame.board[3][4].color) // Flipped piece
        assertEquals(Color.BLACK, newGame.board[4][4].color) // Unchanged piece
        assertEquals(Color.BLACK, newGame.board[3][3].color)
        assertEquals(Color.WHITE, newGame.board[4][3].color)
    ...
    fun `makePlay should update board and switch player for multi-piece and multi-line flips`() {
        val board = makeEmptyBoard().map { it.toMutableList() }
        ...
        // Horizontal
        assertEquals(Color.BLACK, newGame.board[3][4].color)
        assertEquals(Color.BLACK, newGame.board[3][5].color)
        // Vertical
        assertEquals(Color.BLACK, newGame.board[4][3].color)
        assertEquals(Color.BLACK, newGame.board[5][3].color)
        // Diagonal (SE)
        assertEquals(Color.BLACK, newGame.board[4][4].color)
        assertEquals(Color.BLACK, newGame.board[5][5].color)
        // New piece
        assertEquals(Color.BLACK, newGame.board[3][3].color)
    ...
    fun `makePlay should end the game when the board is full and declare a winner`() {
        ...
        val board = makeEmptyBoard().mapIndexed { row, cols ->
        ...
        val game = Reversi(board = board, currentPlayer = Color.BLACK)
        ...
        assertEquals(Reversi.STATE.FINISHED, newGame.state)
    ...
    fun `makePlay should end the game with a draw when both players have the same number of pieces`() {
        ...
        assertEquals(Reversi.STATE.DRAW, newGame.state)
    ...
    fun `makePlay should end the game after consecutive skips`() {
        ...
        val game = Reversi(board = board, currentPlayer = Color.WHITE, state = Reversi.STATE.PLAYING)
