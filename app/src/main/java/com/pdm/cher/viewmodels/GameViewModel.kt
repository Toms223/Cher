...
class GameViewModel(firestore: FirebaseFirestore, auth: FirebaseAuth, storage: FirebaseStorage): ViewModel() {
    private val reversiFirebase = ReversiFirebase(firestore, auth, storage)

    fun getOpponentPlayer(email: String, onResult: (FirebaseResult<Player>) -> Unit) = viewModelScope.launch {
        try {
            onResult(
                FirebaseResult.Success(
                    reversiFirebase.getPlayer(email) ?: throw Exception("Player not found")
                )
            )
        } catch (e: Exception) {
